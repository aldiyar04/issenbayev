...
import kz.iitu.itse1910.issenbayev.repository.entity.*;
...
public interface TicketMapper {
    ...
    @Mapping(target = TicketResolvedDto.Field.PROJECT_NAME, source = Ticket.Field.PROJECT + "." + Project.Field.NAME)
    @Mapping(target = TicketResolvedDto.Field.ASSIGNEE_USERNAME, source = Ticket.Field.ASSIGNEE + "." + User.Field.USERNAME)
    @Mapping(target = TicketResolvedDto.Field.SUBMITTER_USERNAME, source = Ticket.Field.SUBMITTER + "." + User.Field.USERNAME)
    TicketResolvedDto entityToResolvedTicketDto(Ticket ticket);
